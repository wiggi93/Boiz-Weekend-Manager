#!/usr/bin/env node
/**
 * Boiz Weekend Manager — MCP server (stdio).
 *
 * Lets Claude drive the app: create events, toggle modules, start Jeopardy
 * rounds, book kitty expenses, send announcements, …
 *
 * Setup (once):
 ...
 *
 * Env:
 *   BOIZ_EMAIL     — your app login (REQUIRED)
 *   BOIZ_PASSWORD  — your app password (REQUIRED)
 *   BOIZ_PB_URL    — API base (default https://boiz-api.dr-disco.eu)
 *
 * Auth model: the server logs in as YOU, so it can do exactly what you can do
 * in the app — no more. Nothing is exposed publicly; it runs locally over
 * stdio and talks outbound to the API only.
 */
...
/** Run fn, re-authenticating once if the token turned out to be stale. */
...
console.error(`[boiz-mcp] verbunden mit ${PB_URL} als ${EMAIL}`);
